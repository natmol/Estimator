...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // qt state quaternion from the euler angles φ, θ and ψ
  Quaternion<float> qt = qt.FromEuler123_RPY(rollEst,
                                               pitchEst,
                                               ekfState(6));

  // measurement of the angular rates from the IMU in the body frame
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  const double CONST_GRAVITY = 9.81f; // gravity in [m/s^2]

  V3F acceleration = attitude.Rotate_BtoI(accel) + V3F (0.0, 0.0, -CONST_GRAVITY);
  predictedState(0) += predictedState(3) * dt;
  predictedState(1) += predictedState(4) * dt;
  predictedState(2) += predictedState(5) * dt;
  predictedState(3) += acceleration.x * dt;
  predictedState(4) += acceleration.y * dt;
  predictedState(5) += acceleration.z * dt; 
